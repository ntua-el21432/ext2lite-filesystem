                               ...
                               struct buffer_head *bitmap_bh, unsigned long *count)
{
	...
	/* ? */
	first_free_bit=find_next_zero_bit_le(bitmap_bh->b_data,nblocks,group_first_block);
	if (first_free_bit >= nblocks)	{return -1;}
	num=0;
	for(; num< *count && first_free_bit < nblocks; first_free_bit++) {
		if(ext2_set_bit_atomic(sb_bgl_lock(EXT2_SB(sb),group),first_free_bit, bitmap_bh->b_data))
		{
			if(num==0) continue;
			break;
		}
